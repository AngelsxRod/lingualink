...
export const Sidebar = ({ tags, onFilterChange }: SidebarProps) => {
  ...
  return (
    <>
      <Button className="py-2 fixed bottom-20 right-8 z-30 md:hidden" onClick={() => setIsOpen(!isOpen)}>
      ...
      >
        ...
        <div className="mb-8">
          ...
          <ul className="space-y-3">
            {tags.map((tag) => (
              <li key={tag}>
                <label htmlFor={`check-${tag}`} className="flex items-center space-x-3 cursor-pointer">
        ...
        <div>
          ...
          <button onClick={clearSortOrder} className="mt-2 text-sm text-blue-600 underline">
            Limpiar selección
          </button>
        </div>
      </div>
